...
public class SanPhamController {
    ...
    public String addSanPham(@ModelAttribute @Valid SanPham sanPham, BindingResult result, Model model,
                             ...
                             @RequestParam(value = "gia_ban", required = false) BigDecimal giaBan,
        ...
        if (mauSac != null && !mauSac.isEmpty() && kichThuoc != null && !kichThuoc.isEmpty() && chatLieu != null && !chatLieu.isEmpty() && soLuong != null && giaBan != null) {
    ...
    public String softDeleteSanPham(@PathVariable Integer id, RedirectAttributes redirectAttributes) {
        SanPham sp = sanPhamService.getSanPhamById(id);
        if (sp != null) {
            sp.setDeleted(true);
            sanPhamService.updateSanPham(id, sp);
        }
        redirectAttributes.addFlashAttribute("message", "Đã xóa mềm sản phẩm thành công!");
        return "redirect:/san-pham";
    }
